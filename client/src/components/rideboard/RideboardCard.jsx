...
const RideboardCard = ({ trip, onLike, onSave, onJoin, onComment, liked, likeCount, saved, joined, commentCount, continuous, onDeleteComment, currentUserId }) => {
  ...
  return (
    <section
      className={`w-full bg-white ${continuous ? "rounded-none border-b border-gray-200" : "rounded-2xl shadow-lg"} flex flex-col gap-0 overflow-hidden`}
    ...
    >
      ...
      <div className="px-6 pt-4 pb-2 flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-4 mt-4 mb-2">
          ...
          <button
            onClick={onSave}
            className={`flex items-center gap-1 px-3 py-1 rounded-full font-semibold transition border-none bg-transparent text-blue-700 hover:bg-blue-50 focus:outline-none ${saved ? "bg-blue-100" : ""}`}
            style={{ boxShadow: "none" }}
          >
            <Bookmark size={20} fill={saved ? "#2563eb" : "none"} stroke="#2563eb" />
            <span>{trip.saves?.length || 0}</span>
          </button>
          <button
            onClick={onJoin}
            className={`flex items-center gap-1 px-3 py-1 rounded-full font-semibold transition border-none bg-transparent text-green-700 hover:bg-green-50 focus:outline-none ${joined ? "bg-green-100" : ""}`}
            style={{ boxShadow: "none" }}
          >
            <Users size={20} fill={joined ? "#22c55e" : "none"} stroke="#22c55e" />
            <span>{trip.joins?.length || 0}</span>
          </button>
        </div>
        {/* Comment List */}
        <div className="mt-2">
        ...
        <form onSubmit={handleCommentSubmit} className="flex items-center gap-2 mt-2 border-t border-gray-200 pt-2">
          <input
            type="text"
            className="flex-1 rounded-full px-4 py-2 bg-gray-100 focus:bg-white border border-gray-200 focus:border-indigo-400 outline-none text-sm"
